...
- Auth: JWT (jsonwebtoken) + server-side `sessions` collection storing only SHA-256 token hashes; TTL index auto-expires sessions. RBAC: `faculty` / `hod` / `admin`. No seeded user accounts and no guest mode — login is mandatory; accounts are created in-app via `POST /api/auth/register` (role `teacher` → `faculty`, role `hod` → Principal/HOD; teachers are linked to or auto-created as a faculty record).
- `GET /api/meta` (universities → colleges → departments) is a union of the `colleges` collection (seeded from `server/src/colleges-data.js` — 98 Maharashtra universities, 150 colleges) and the `faculties` collection (demo orgs University of Metro / Global Institute of Technology).
...
- Onboarding entry: `src/pages/Onboarding.jsx` — first page has University + College dropdowns (placeholders "Pick University Name" / "Pick college name"), then Sign in / Create account. Registration lives in `src/components/onboarding/RegisterStep.jsx` (role selector: Teacher vs Principal/HOD; department select with free-text fallback when a college has no departments).